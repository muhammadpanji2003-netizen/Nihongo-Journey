...
function learn(){
  const steps = [
    {
      icon:"🌱", title:"Beginner", desc:"Pengenalan alur belajar dan fondasi bahasa Jepang.",
      href:"#/placement", status:"Mulai", cls:"current"
    },
    {
      icon:"あ", title:"Hiragana", desc:"Belajar huruf → kuis 50 soal → latihan random tanpa batas → review.",
      href:"#/learn/kana?type=hiragana", status:"Tersedia", cls:""
    },
    {
      icon:"ア", title:"Katakana", desc:"Belajar huruf → kuis 50 soal → latihan random tanpa batas → review.",
      href:"#/learn/kana?type=katakana", status:"Tersedia", cls:""
    },
    {
      icon:"🇯🇵", title:"JLPT N5", desc:"Vocabulary, Kanji, Grammar, Listening, dan Reading level N5.",
      href:"#/learn/level/N5", status:"Tersedia", cls:""
    },
    {
      icon:"🇯🇵", title:"JLPT N4", desc:"Materi dasar-menengah: kosakata, Kanji, grammar, listening, dan reading.",
      href:"#/learn/level/N4", status:"Tersedia", cls:""
    },
    {
      icon:"🇯🇵", title:"JLPT N3", desc:"Materi menengah dan persiapan kemampuan menuju JLPT N3.",
      href:"#/learn/level/N3", status:"Tersedia", cls:""
    },
    {
      icon:"🏆", title:"N3 Master", desc:"Penguatan area lemah, review, dan persiapan mock test.",
      href:"#/learn/n3-master", status:"Segera", cls:"coming"
    }
  ];

  app.innerHTML = `${header("BELAJAR","Learning Path","Klik tahap mana pun untuk langsung membuka materi. Login tidak wajib untuk belajar.")}
  <div class="roadmap clickable-roadmap">
    ${steps.map(s=>`
      <a class="road-step ${s.cls} road-link" href="${s.href}" aria-label="Buka ${s.title}">
        <div class="road-icon">${s.icon}</div>
        <div class="road-copy"><h3>${s.title}</h3><p>${s.desc}</p></div>
        <div class="road-action"><span class="badge blue">${s.status}</span><span class="road-arrow">→</span></div>
      </a>`).join("")}
  </div>

  <section class="section">
    ${header("MATERI","Pilih kategori","Kamu juga bisa langsung membuka kategori materi tanpa mengikuti urutan Learning Path.")}
  ...
  <div class="notice section"><strong>Tanpa akun:</strong> seluruh materi, kuis, dan latihan tetap bisa digunakan. <strong>Dengan akun:</strong> progres belajar disimpan online dan dapat dilanjutkan di perangkat lain.</div>

  ${state.wrong.length ? `<section class="section"><div class="card review-inline"><div><small class="eyebrow">REVIEW SAAT BELAJAR</small><h2>${state.wrong.length} item perlu diulang</h2><p>Review muncul di dalam alur belajar, bukan sebagai menu utama.</p></div><button class="btn primary" id="inlineReview">Mulai Review</button></div></section>` : ""}`;
  if($("#inlineReview")) $("#inlineReview").onclick=()=>review();
}

function levelPage(level){
  const v=vocab.filter(x=>x.level===level);
  const k=kanji.filter(x=>x.level===level);
  const g=grammar.filter(x=>x.level===level);
  const r=readings.filter(x=>x.level===level);
  app.innerHTML=`${header(level,`Belajar ${level}`,"Pilih bagian materi yang ingin dipelajari. Semua bagian dapat dibuka tanpa login.")}
  <div class="grid two">
    <a class="card hover module-card" href="#/learn/vocab"><div class="icon">📖</div><h3>Vocabulary ${level}</h3><p>${v.length} kosakata tersedia pada level ini.</p></a>
    <a class="card hover module-card" href="#/learn/kanji"><div class="icon">🈶</div><h3>Kanji ${level}</h3><p>${k.length} Kanji tersedia pada level ini.</p></a>
    <a class="card hover module-card" href="#/learn/grammar"><div class="icon">🧩</div><h3>Grammar ${level}</h3><p>${g.length} pola grammar tersedia pada level ini.</p></a>
    <a class="card hover module-card" href="#/learn/reading"><div class="icon">📚</div><h3>Reading ${level}</h3><p>${r.length} bacaan tersedia pada level ini.</p></a>
    <a class="card hover module-card" href="#/learn/listening"><div class="icon">🎧</div><h3>Listening ${level}</h3><p>Latihan mendengar menggunakan materi bahasa Jepang.</p></a>
    <a class="card hover module-card" href="#/learn"><div class="icon">←</div><h3>Kembali ke Learning Path</h3><p>Pilih tahap belajar lainnya.</p></a>
  </div>`;
}

function n3Master(){
  app.innerHTML=`${header("N3 MASTER","N3 Master","Tahap penguatan setelah menyelesaikan materi N3.")}
  <div class="card">
    <span class="badge blue">Dalam pengembangan</span>
    <h2 style="margin-top:12px">Review & Mock Test</h2>
    <p>Bagian ini disiapkan untuk mock test, review area lemah, dan evaluasi akhir. Untuk sekarang, lanjutkan materi N3 terlebih dahulu.</p>
    <a class="btn primary" href="#/learn/level/N3">Buka Materi N3</a>
  </div>`;
}

function kanaPage(){
 let params=new URLSearchParams((location.hash.split("?")[1]||""));
 let type=params.get("type")==="katakana"?"katakana":"hiragana", group="basic";
 ...
 <div class="tabs"><button class="tab ${type==="hiragana"?"active":""}" data-type="hiragana">Hiragana</button><button class="tab ${type==="katakana"?"active":""}" data-type="katakana">Katakana</button></div>
...
function render(){
 ...
 else if(r.startsWith("/learn/kana")) kanaPage();
 else if(r==="/learn/level/N5") levelPage("N5");
 else if(r==="/learn/level/N4") levelPage("N4");
 else if(r==="/learn/level/N3") levelPage("N3");
 else if(r==="/learn/n3-master") n3Master();
